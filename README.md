...
learning project to understand the internals of modern language models — able to
load OpenAI's pretrained GPT-2 weights to generate text, and to run a training
...
- **Data batching** — tokenizes a plain-text corpus (`notebooks/input.txt`, Tiny Shakespeare) with `tiktoken`'s GPT-2 encoder and builds `(input, target)` batches for next-token prediction
